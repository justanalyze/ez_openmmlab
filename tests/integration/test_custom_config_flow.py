...
@patch("cv2.imread")
def test_full_custom_config_flow(
    mock_imread,
    mock_inferencer_cls,
    mock_ds_from_toml,
    mock_ensure,
    ...
    with patch(
        ...
        return_value=custom_checkpoint,
    ):
        custom_detector = RTMDet(
            model=saved_config_path, checkpoint_path=custom_checkpoint
        )
        ...
        mock_inferencer_instance.return_value = {
            "predictions": [
                {"labels": [0], "scores": [0.9], "bboxes": [[10, 10, 50, 50]]}
            ]
        }
