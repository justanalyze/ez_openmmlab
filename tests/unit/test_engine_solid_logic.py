...
class ConcreteMockEngine(EZMMLab):
    def __init__(self, model="mock_model"):
        ...
        with patch("ez_openmmlab.core.engines.engine_base.ensure_model_checkpoint", return_value=Path("mock.pth")):
            with patch("ez_openmmlab.core.engines.engine_base.get_config_file", return_value=Path("mock.py")):
